...
function populateZInputs() {
  ...
  if (zData.workerZone) {
    $('ze-w-lm').value = (zData.machineZone.leftMarginFrac).toFixed(4);
...
function syncInputsToData() {
  ...
  if (zActiveTab === 'w') {
    zData.machineZone.leftMarginFrac = val('ze-w-lm'); // shared left margin
...
function renderWorkerZone() {
  ...
  const mShared = zData.machineZone; // shares left margin
  const lx = mShared.leftMarginFrac * zw; 
  const my = m.startYFrac * zh;
  const rx = zw;
  ...
  createHandle(lx, my, 'wtl', (dx,dy) => {
    mShared.leftMarginFrac = Math.max(0, lx + dx)/zw;
    m.startYFrac = Math.max(0, my + dy)/zh;
  });
}
